   ...
   PottsAmPathParser(spk,narg,arg),  MobilityOut(0)
{
   ...
   exp_factor = atof(arg[11]); //Exponential parameter for mobility decay in haz M(d) = exp(-exp_factor * d)
...
void AppPottsAdditive::app_update(double dt)
{
   ...
   double d;
	for(int i=0;i<nlocal;i++){
		...
		if(xo[0] < 0 && ((xo[2] <= 0 && abs(xo[2]) <= depth_HAZ)||(xo[2]>0 && abs(xo[2]) <= melt_cap))  && xo[0] > -tail_HAZ && abs(xo[1]) <= HAZ/2.0) {
		...
		else if (abs(xo[0]) <= cap_HAZ && ((xo[2] <=0 && abs(xo[2]) <= depth_HAZ)||(xo[2]>0 && abs(xo[2]) <= melt_cap)) && abs(xo[1]) <= HAZ/2.0) {
			d = ae.distance(xo);
		}
		else d = -10;
